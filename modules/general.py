...
def log_summary(item_name: str, checked_count: int, found_count: int) -> None:
    ...
    if found_count:
        ...
        message_format = QCoreApplication.translate(
...
class LayerManager:
    ...
    def new_layer(self) -> QgsVectorLayer:
        ...
        if self._new_layer is None:
            raise_runtime_error(
                QCoreApplication.translate("RuntimeError", "New layer is not set.")
            )
    ...
    def reproject_layer_to_project_crs(self, layer: QgsVectorLayer) -> QgsVectorLayer:
        ...
        if data_provider is None:
            raise_runtime_error(
                QCoreApplication.translate(
                    "RuntimeError", "Could not get data provider for layer: {0}"
    ...
    def get_selected_layer(self) -> QgsVectorLayer:
        ...
        if not layer_tree:
            raise_runtime_error(
                QCoreApplication.translate(
                    "RuntimeError", "Could not get layer tree view."
                )
            )
    ...
    def create_new_layer(self) -> QgsVectorLayer:
        ...
        log_debug(
            QCoreApplication.translate(
                "log", "Added {0} from the GeoPackage to the project."
